...
var game = {
  ...
  start: function(){
    ...
    $('#startGame, .game').remove();
    for(var i=0; i<questions.length; i++){
      $('#questionaire').append('<br><h3>' + questions[i].question + '</h3>')
      for(var j=0; j < questions[i].choices.length; j++){
        $('#questionaire').append("<div class ='userInput'><input class='button' type='radio' name='q"+ i +"' value='"+questions[i].choices[j]+"'><label class='label'>"+ questions[i].choices[j]+"  </label></div>");
      }
    }
  ...
  result: function(){
    ...
    if (game.correct >= 6){
      $('#questionaire').html('<h2>Looks like there\'s a new sheriff in town!</h2> <br> <img src="assets/images/q8.gif" alt="high"><br>').addClass('finished');
    }
    else if (game.correct >=4){
      $('#questionaire').html('<h2>Better Luck Next Time, Cowboy!</h2> <br> <img src="assets/images/q2.gif" alt="mid"><br>').addClass('finished');
    }
    else {
      $('#questionaire').html('<h2>Best be leavin\' til ya know what you\'re doin\'!<br> <img src="assets/images/q1.gif" alt="low"><br>').addClass('finished');
    }
    $('#questionaire').append("<h3>Questions Ya Done Well: "+this.correct+"</h3>");
    $('#questionaire').append("<h3>Questions Ya Ain't Done Well: "+this.incorrect+"</h3>");
    // display all the Unanswered questions
    $('#questionaire').append("<h3>Questions You Didn't Round Up: "+(questions.length-(this.incorrect+this.correct))+"<h3>");
...
$('#completedGame').on('click', function(){
  game.countDown();
});